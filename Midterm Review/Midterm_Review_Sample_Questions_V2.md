...
| 12 | B) `guard let unwrappedValue = optionalValue else { return }` |
...
| 15 | B) Inline initialization happens for all instances; init() can vary based on parameters |
...
| 30 | B) In the Parent's `prepare(for:sender:)` method |
